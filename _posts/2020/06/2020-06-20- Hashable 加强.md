...
[FNV-1a]: https://www.isthe.com/chongo/tech/comp/fnv/index.html
...
variable; see [Effect on ABI stability] for details.)
-->

（随机种子可以通过特定的环境变量进行关闭；详情请看[对于 ABI 稳定的影响]。）
...
compatibility] on how we'll do this without
...
同时，我们会将 `hashValue` 属性标记为弃用。（请看[代码稳定]小节，了解我们如何在不破坏旧版 Swift 代码的同时做到这一点。）在未来的版本里，我们倾向于把 `hashValue` 转换为一个 extension 方法。
...
is explained in the section on [The `hash(into:)` requirement].
-->

我们觉得这个做法并不那么令人满意；根本原因在 [实现要求] 里有比较详细的说明。
...
### 让 `Hasher` 成为一个协议，使得  `hash(into:)` 变成一个泛型函数
