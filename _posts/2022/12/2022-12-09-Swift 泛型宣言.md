...
像这样的信息可以在[独立的讨论进程](https://www.urbandictionary.com/define.php?term=centithread)里进行。为了让我们的讨论尽可能独立，我会要求讨论进程里只讨论主题功能的愿景：如何让各个设计更好得融合到一起，还缺乏哪些设计，这些设计是否符合 Swift 的长期愿景，诸如此类。关于特定语言功能的讨论，例如，Conditional Conformance 的语法和语义，或者是编译器的实现，标准库的使用，请重新开一个讨论进程，并且使用的官方对于该功能的称谓。
...
可变泛型可以允许我们把一系列的泛型参数抽象出来。下面的语法无可救药地被 [C++11 可变模版](https://www.jot.fm/issues/issue_2008_02/article2/)影响（抱歉），在声明的左边加上一个省略号（“...”），让它成为一个“参数集合“，可以包含零到多个参数；把省略号放在类型/表达式的右边，可以把带类型和表达式的参数集合展开成单独的参数。重要的是我们终于可以把泛型参数的集合抽象出来了：
